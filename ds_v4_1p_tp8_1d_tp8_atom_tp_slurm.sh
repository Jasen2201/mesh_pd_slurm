#!/usr/bin/env bash
#SBATCH --job-name=ds-v4-1p-tp8-1d-tp8-atom-tp
...
#SBATCH --output=/it-share/yajizhan/slurm_dsv4_logs/ds_v4_1p_tp8_1d_tp8_atom_tp-%j.out
#SBATCH --error=/it-share/yajizhan/slurm_dsv4_logs/ds_v4_1p_tp8_1d_tp8_atom_tp-%j.err
#
...
CONTAINER="${CONTAINER:-atom_mesh_dsv4_1p1d_tp_${SLURM_JOB_ID}}"
...
LOG_ROOT="${LOG_ROOT:-/it-share/yajizhan/slurm_dsv4_logs/$(date +%m%d)_ds_v4_1p_tp8_1d_tp8_atom_tp_${SLURM_JOB_ID}}"
...
for GSM8K_CONC in "${GSM8K_CONCS[@]}"; do
    RUN_TAG="$(date +%Y%m%d%H%M%S)_gsm8k_v4_1p1d_tp_c${GSM8K_CONC}"
    echo ""
    echo "========================================="
...
for ISL in "${ISLS[@]}"; do
    for CONC in "${CONCS[@]}"; do
        RESULT_FILENAME="pd-atom-v4-1p1d-tp-${ISL}-${OSL}-${CONC}-${RANDOM_RANGE_RATIO}"
        echo ""
        echo "========================================="
...
json_files = sorted(result_dir.glob('pd-atom-v4-1p1d-tp-*.json'))
if not json_files:
...
launch_container() {
    ...
    srun --nodelist="$node" --nodes=1 --ntasks=1 bash -lc "
        ...
        # Tune TCP backlog for high-concurrency PD serving
