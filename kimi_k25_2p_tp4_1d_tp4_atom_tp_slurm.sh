#!/usr/bin/env bash
#SBATCH --job-name=kimi-k25-2p-tp4-1d-tp4-atom-tp
...
#SBATCH --nodelist=mia1-p02-g44,mia1-p02-g47
#SBATCH --output=/it-share/yajizhan/slurm_kimi_k25_logs/kimi_k25_2p_tp4_1d_tp4_atom_tp-%j.out
#SBATCH --error=/it-share/yajizhan/slurm_kimi_k25_logs/kimi_k25_2p_tp4_1d_tp4_atom_tp-%j.err
#
# 2P+1D PD-disaggregated benchmark for Kimi-K2.5-MXFP4
# on ATOM with mooncake RDMA KV transfer.
#   prefill: 2x TP=4 on node1 (GPU 0-3 + GPU 4-7), decode: TP=4 on node2 (GPU 0-3).
#
# Usage:
#   mkdir -p /it-share/yajizhan/slurm_kimi_k25_logs
#   sbatch kimi_k25_2p_tp4_1d_tp4_atom_tp_slurm.sh

set -euo pipefail

# ======================== configuration ========================
MODEL_PATH="${MODEL_PATH:-/mnt/models/Kimi-K2.5-MXFP4/}"
DOCKER_IMAGE="${DOCKER_IMAGE:-rocm/atom-dev:latest}"
CONTAINER="${CONTAINER:-atom_mesh_kimi_k25_2p1d_tp4_${SLURM_JOB_ID}}"

PREFILL_TP="${PREFILL_TP:-4}"
DECODE_TP="${DECODE_TP:-4}"
PREFILL_PORT_1="${PREFILL_PORT_1:-8010}"
...
OSL="${OSL:-1024}"
CONC_LIST="${CONC_LIST:-128,256,768,1024}"
...
GSM8K_NUM_CONCURRENT="${GSM8K_NUM_CONCURRENT:-128}"

LOG_ROOT="${LOG_ROOT:-/it-share/yajizhan/slurm_kimi_k25_logs/$(date +%m%d)_kimi_k25_2p_tp4_1d_tp4_atom_tp_${SLURM_JOB_ID}}"
...
DECODE_NODE="${NODES[1]}"
ALL_NODES=("$PREFILL_NODE" "$DECODE_NODE")
...
cat <<INFO
=== Configuration ===
PREFILL-1 : ${PREFILL_NODE} (IP=${PREFILL_IP}, TP=${PREFILL_TP}, GPU=0-3, port=${PREFILL_PORT_1})
PREFILL-2 : ${PREFILL_NODE} (IP=${PREFILL_IP}, TP=${PREFILL_TP}, GPU=4-7, port=${PREFILL_PORT_2})
DECODE    : ${DECODE_NODE}  (IP=${DECODE_IP},  TP=${DECODE_TP},  GPU=0-3, port=${DECODE_PORT})
ROUTER    : ${PREFILL_IP}:${ROUTER_PORT}
MODEL     : ${MODEL_PATH}
IMAGE     : ${DOCKER_IMAGE}
BACKEND   : atom (PD mooncake KV transfer, 2P+1D TP4)
...
cat > "${LOG_ROOT}/scripts/prefill_1.sh" <<'PREFILL1_EOF'
#!/usr/bin/env bash
set -euo pipefail

echo "[prefill-1] IP=${PREFILL_IP} TP=${PREFILL_TP} GPU=0-3 port=${PREFILL_PORT_1}"
...
python3 -m atom.entrypoints.openai_server \
    ...
    --kv-transfer-config '{"kv_role":"kv_producer","kv_connector":"mooncake","proxy_ip":"${PREFILL_IP}","handshake_port":${HANDSHAKE_PORT_1}}' \
    ${EXTRA_SERVER_ARGS} \
    2>&1 | tee /workspace/logs/prefill_1.log
PREFILL1_EOF

cat > "${LOG_ROOT}/scripts/prefill_2.sh" <<'PREFILL2_EOF'
#!/usr/bin/env bash
set -euo pipefail

echo "[prefill-2] IP=${PREFILL_IP} TP=${PREFILL_TP} GPU=4-7 port=${PREFILL_PORT_2}"
...
python3 -m atom.entrypoints.openai_server \
    ...
    --gpu-memory-utilization "${MEM_FRACTION}" \
    --max-num-seqs "${MAX_NUM_SEQS}" \
    --kv-transfer-config '{"kv_role":"kv_producer","kv_connector":"mooncake","proxy_ip":"${PREFILL_IP}","handshake_port":${HANDSHAKE_PORT_2}}' \
    ${EXTRA_SERVER_ARGS} \
    2>&1 | tee /workspace/logs/prefill_2.log
PREFILL2_EOF
...
echo "[decode] IP=${DECODE_IP} TP=${DECODE_TP} GPU=0-3 port=${DECODE_PORT}"
...
python3 -m atom.entrypoints.openai_server \
    ...
    --gpu-memory-utilization "${MEM_FRACTION}" \
    --max-num-seqs "${MAX_NUM_SEQS}" \
    --kv-transfer-config '{"kv_role":"kv_consumer","kv_connector":"mooncake","proxy_ip":"${DECODE_IP}","handshake_port":${HANDSHAKE_PORT_1}}' \
...
echo "[router] prefill-1=${PREFILL_IP}:${PREFILL_PORT_1}"
echo "[router] prefill-2=${PREFILL_IP}:${PREFILL_PORT_2}"
...
/usr/local/bin/atomesh launch \
    ...
    --pd-disaggregation \
    --prefill "http://${PREFILL_IP}:${PREFILL_PORT_1}" \
    --prefill "http://${PREFILL_IP}:${PREFILL_PORT_2}" \
...
for GSM8K_CONC in "${GSM8K_CONCS[@]}"; do
    RUN_TAG="$(date +%Y%m%d%H%M%S)_gsm8k_kimi_k25_2p1d_tp4_c${GSM8K_CONC}"
    echo ""
    echo "========================================="
...
for ISL in "${ISLS[@]}"; do
    for CONC in "${CONCS[@]}"; do
        RESULT_FILENAME="pd-atom-kimi-k25-2p1d-tp4-${ISL}-${OSL}-${CONC}-${RANDOM_RANGE_RATIO}"
        echo ""
        echo "========================================="
...
json_files = sorted(result_dir.glob('pd-atom-kimi-k25-2p1d-tp4-*.json'))
if not json_files:
...
for script in "${LOG_ROOT}"/scripts/*.sh; do
    sed -i \
        ...
        -e "s|\${PREFILL_PORT_1}|${PREFILL_PORT_1}|g" \
        -e "s|\${PREFILL_PORT_2}|${PREFILL_PORT_2}|g" \
        -e "s|\${DECODE_PORT}|${DECODE_PORT}|g" \
        -e "s|\${ROUTER_PORT}|${ROUTER_PORT}|g" \
        -e "s|\${HANDSHAKE_PORT_1}|${HANDSHAKE_PORT_1}|g" \
        -e "s|\${HANDSHAKE_PORT_2}|${HANDSHAKE_PORT_2}|g" \
...
# ======================== 1. start containers ========================
launch_container "$PREFILL_NODE" prefill_1
launch_container "$DECODE_NODE"  decode

# ======================== 2. start prefill servers (detached, both on same node) ========================
echo "[prefill-1] launching server on ${PREFILL_NODE} (GPU 0-3)"
srun --nodelist="$PREFILL_NODE" --nodes=1 --ntasks=1 bash -lc "
    docker exec -d '${CONTAINER}' bash '${LOG_ROOT}/scripts/prefill_1.sh'
"
echo "[prefill-2] launching server on ${PREFILL_NODE} (GPU 4-7)"
srun --nodelist="$PREFILL_NODE" --nodes=1 --ntasks=1 bash -lc "
...
# ======================== 4. wait for servers ========================
wait_endpoint "$PREFILL_NODE" "http://${PREFILL_IP}:${PREFILL_PORT_1}/health" \
    "$WAIT_SERVER_TIMEOUT" "prefill-1-http"
wait_endpoint "$PREFILL_NODE" "http://${PREFILL_IP}:${PREFILL_PORT_2}/health" \
