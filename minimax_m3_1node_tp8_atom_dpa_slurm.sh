#!/usr/bin/env bash
#SBATCH --job-name=minimax-m3-1node-tp8-atom-nodpa
...
#SBATCH --output=/it-share/yajizhan/slurm_minimax_logs/minimax_m3_1node_tp8_atom_nodpa-%j.out
#SBATCH --error=/it-share/yajizhan/slurm_minimax_logs/minimax_m3_1node_tp8_atom_nodpa-%j.err
#
# Single-node accuracy check for MiniMax-M3-MXFP4 on ATOM without DPA/EP.
#   No PD disaggregation — plain TP=8 server (no dp-attention, no expert-parallel) + GSM8K eval.
#   Use this to isolate precision issues from the DPA/EP path.
#
# Usage:
#   mkdir -p /it-share/yajizhan/slurm_minimax_logs
#   sbatch minimax_m3_1node_tp8_atom_dpa_slurm.sh
...
CONTAINER="${CONTAINER:-atom_minimax_m3_1node_nodpa_${SLURM_JOB_ID}}"
...
LOG_ROOT="${LOG_ROOT:-/it-share/yajizhan/slurm_minimax_logs/$(date +%m%d)_minimax_m3_1node_tp8_atom_nodpa_${SLURM_JOB_ID}}"
...
for GSM8K_CONC in "${GSM8K_CONCS[@]}"; do
    RUN_TAG="$(date +%Y%m%d%H%M%S)_gsm8k_minimax_m3_1node_dpa_c${GSM8K_CONC}"
    echo ""
    echo "========================================="
...
for ISL in "${ISLS[@]}"; do
    for CONC in "${CONCS[@]}"; do
        RESULT_FILENAME="atom-minimax-m3-1node-dpa-${ISL}-${OSL}-${CONC}-${RANDOM_RANGE_RATIO}"
        echo ""
        echo "========================================="
...
json_files = sorted(result_dir.glob('atom-minimax-m3-1node-dpa-*.json'))
if not json_files:
