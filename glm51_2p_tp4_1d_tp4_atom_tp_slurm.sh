#!/usr/bin/env bash
#SBATCH --job-name=glm51-2p-tp4-1d-tp4-atom-tp
...
#SBATCH --output=/it-share/yajizhan/slurm_glm51_logs/glm51_2p_tp4_1d_tp4_atom_tp-%j.out
#SBATCH --error=/it-share/yajizhan/slurm_glm51_logs/glm51_2p_tp4_1d_tp4_atom_tp-%j.err
#
# 2P+1D PD-disaggregated benchmark for GLM-5.1-MXFP4
...
#   sbatch glm51_2p_tp4_1d_tp4_atom_tp_slurm.sh
...
CONTAINER="${CONTAINER:-atom_mesh_glm51_2p1d_tp4_${SLURM_JOB_ID}}"
...
EXTRA_SERVER_ARGS="${EXTRA_SERVER_ARGS:---default-chat-template-kwargs '{\"enable_thinking\":false}'}"
...
LOG_ROOT="${LOG_ROOT:-/it-share/yajizhan/slurm_glm51_logs/$(date +%m%d)_glm51_2p_tp4_1d_tp4_atom_tp_${SLURM_JOB_ID}}"
...
for GSM8K_CONC in "${GSM8K_CONCS[@]}"; do
    RUN_TAG="$(date +%Y%m%d%H%M%S)_gsm8k_glm51_2p1d_tp4_c${GSM8K_CONC}"
    echo ""
    echo "========================================="
...
for ISL in "${ISLS[@]}"; do
    for CONC in "${CONCS[@]}"; do
        RESULT_FILENAME="pd-atom-glm51-2p1d-tp4-${ISL}-${OSL}-${CONC}-${RANDOM_RANGE_RATIO}"
        echo ""
        echo "========================================="
...
json_files = sorted(result_dir.glob('pd-atom-glm51-2p1d-tp4-*.json'))
if not json_files:
