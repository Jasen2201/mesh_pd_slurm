#!/usr/bin/env bash
#SBATCH --job-name=glm51-1p-tp8-1d-tp8-atom-tp
...
#SBATCH --output=/it-share/yajizhan/slurm_glm51_logs/glm51_1p_tp8_1d_tp8_atom_tp-%j.out
#SBATCH --error=/it-share/yajizhan/slurm_glm51_logs/glm51_1p_tp8_1d_tp8_atom_tp-%j.err
#
...
#   sbatch glm51_1p_tp8_1d_tp8_atom_tp_slurm.sh
...
CONTAINER="${CONTAINER:-atom_mesh_glm51_1p1d_tp_${SLURM_JOB_ID}}"
...
LOG_ROOT="${LOG_ROOT:-/it-share/yajizhan/slurm_glm51_logs/$(date +%m%d)_glm51_1p_tp8_1d_tp8_atom_tp_${SLURM_JOB_ID}}"
...
for GSM8K_CONC in "${GSM8K_CONCS[@]}"; do
    RUN_TAG="$(date +%Y%m%d%H%M%S)_gsm8k_glm51_1p1d_tp_c${GSM8K_CONC}"
    echo ""
    echo "========================================="
...
for ISL in "${ISLS[@]}"; do
    for CONC in "${CONCS[@]}"; do
        RESULT_FILENAME="pd-atom-glm51-1p1d-tp-${ISL}-${OSL}-${CONC}-${RANDOM_RANGE_RATIO}"
        echo ""
        echo "========================================="
...
json_files = sorted(result_dir.glob('pd-atom-glm51-1p1d-tp-*.json'))
if not json_files:
