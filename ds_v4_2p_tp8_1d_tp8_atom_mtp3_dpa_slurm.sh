#!/usr/bin/env bash
#SBATCH --job-name=ds-v4-2p-tp8-1d-tp8-atom-mtp3-dpa
...
#SBATCH --output=/it-share/yajizhan/slurm_dsv4_logs/ds_v4_2p_tp8_1d_tp8_atom_mtp3_dpa-%j.out
#SBATCH --error=/it-share/yajizhan/slurm_dsv4_logs/ds_v4_2p_tp8_1d_tp8_atom_mtp3_dpa-%j.err
#
...
#   sbatch ds_v4_2p_tp8_1d_tp8_atom_mtp3_dpa_slurm.sh
...
CONTAINER="${CONTAINER:-atom_mesh_dsv4_2p1d_mtp3_dpa_${SLURM_JOB_ID}}"
...
OSL="${OSL:-1024}"
CONC_LIST="${CONC_LIST:-512,768,1024,2048}"
...
LOG_ROOT="${LOG_ROOT:-/it-share/yajizhan/slurm_dsv4_logs/$(date +%m%d)_ds_v4_2p_tp8_1d_tp8_atom_mtp3_dpa_${SLURM_JOB_ID}}"
...
ln -sfn "${LOG_ROOT}" /it-share/yajizhan/slurm_dsv4_logs/latest_ds_v4_2p1d_mtp3_dpa
...
for GSM8K_CONC in "\${GSM8K_CONCS[@]}"; do
    RUN_TAG="\$(date +%Y%m%d%H%M%S)_gsm8k_v4_2p1d_mtp3_dpa_c\${GSM8K_CONC}"
    echo ""
    echo "========================================="
...
for ISL in "\${ISLS[@]}"; do
    for CONC in "\${CONCS[@]}"; do
        RESULT_FILENAME="pd-atom-v4-2p1d-mtp3-dpa-\${ISL}-${OSL}-\${CONC}-${RANDOM_RANGE_RATIO}"
        echo ""
        echo "========================================="
...
json_files = sorted(result_dir.glob('pd-atom-v4-2p1d-mtp3-dpa-*.json'))
if not json_files:
