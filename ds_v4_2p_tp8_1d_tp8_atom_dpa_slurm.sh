#!/usr/bin/env bash
#SBATCH --job-name=ds-v4-2p-tp8-1d-tp8-atom-dpa
...
#SBATCH --output=/it-share/yajizhan/slurm_dsv4_logs/ds_v4_2p_tp8_1d_tp8_atom_dpa-%j.out
#SBATCH --error=/it-share/yajizhan/slurm_dsv4_logs/ds_v4_2p_tp8_1d_tp8_atom_dpa-%j.err
#
...
LOG_ROOT="${LOG_ROOT:-/it-share/yajizhan/slurm_dsv4_logs/$(date +%m%d)_ds_v4_2p_tp8_1d_tp8_atom_dpa_${SLURM_JOB_ID}}"
...
for GSM8K_CONC in "${GSM8K_CONCS[@]}"; do
    RUN_TAG="$(date +%Y%m%d%H%M%S)_gsm8k_v4_2p1d_dpa_c${GSM8K_CONC}"
    echo ""
    echo "========================================="
...
for ISL in "${ISLS[@]}"; do
    for CONC in "${CONCS[@]}"; do
        RESULT_FILENAME="pd-atom-v4-2p1d-dpa-${ISL}-${OSL}-${CONC}-${RANDOM_RANGE_RATIO}"
        echo ""
        echo "========================================="
...
json_files = sorted(result_dir.glob('pd-atom-v4-2p1d-dpa-*.json'))
if not json_files:
