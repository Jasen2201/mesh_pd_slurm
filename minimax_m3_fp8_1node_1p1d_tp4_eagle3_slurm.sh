#!/usr/bin/env bash
#SBATCH --job-name=minimax-m3-fp8-1node-1p1d-tp4-eagle3
...
#   sbatch minimax_m3_fp8_1node_1p1d_tp4_eagle3_slurm.sh
...
CONTAINER="${CONTAINER:-atom_mesh_minimax_m3_fp8_1node_1p1d_tp4_eagle3_${SLURM_JOB_ID}}"
...
LOG_ROOT="${LOG_ROOT:-/it-share/yajizhan/slurm_minimax_logs/$(date +%m%d)_minimax_m3_fp8_1node_1p1d_tp4_eagle3_${SLURM_JOB_ID}}"
...
cat <<INFO
=== Configuration (single-node 1P+1D FP8 + EAGLE3) ===
...
for GSM8K_CONC in "${GSM8K_CONCS[@]}"; do
    RUN_TAG="$(date +%Y%m%d%H%M%S)_gsm8k_minimax_m3_fp8_1node_1p1d_tp4_eagle3_c${GSM8K_CONC}"
    echo ""
    echo "========================================="
...
for ISL in "${ISLS[@]}"; do
    for CONC in "${CONCS[@]}"; do
        RESULT_FILENAME="pd-atom-minimax-m3-fp8-1node-1p1d-tp4-eagle3-${ISL}-${OSL}-${CONC}-${RANDOM_RANGE_RATIO}"
        echo ""
        echo "========================================="
...
json_files = sorted(result_dir.glob('pd-atom-minimax-m3-fp8-1node-1p1d-tp4-eagle3-*.json'))
if not json_files:
