#!/usr/bin/env bash
#SBATCH --job-name=kimi-k25-1p-tp4-1d-tp4-atom-tp
...
#SBATCH --output=/it-share/yajizhan/slurm_kimi_k25_logs/kimi_k25_1p_tp4_1d_tp4_atom_tp-%j.out
#SBATCH --error=/it-share/yajizhan/slurm_kimi_k25_logs/kimi_k25_1p_tp4_1d_tp4_atom_tp-%j.err
#
# 1P+1D PD-disaggregated benchmark for Kimi-K2.5-MXFP4
...
#   sbatch kimi_k25_1p_tp4_1d_tp4_atom_tp_slurm.sh
...
CONTAINER="${CONTAINER:-atom_mesh_kimi_k25_1p1d_tp4_${SLURM_JOB_ID}}"
...
LOG_ROOT="${LOG_ROOT:-/it-share/yajizhan/slurm_kimi_k25_logs/$(date +%m%d)_kimi_k25_1p_tp4_1d_tp4_atom_tp_${SLURM_JOB_ID}}"
...
for GSM8K_CONC in "${GSM8K_CONCS[@]}"; do
    RUN_TAG="$(date +%Y%m%d%H%M%S)_gsm8k_kimi_k25_1p1d_tp4_c${GSM8K_CONC}"
    echo ""
    echo "========================================="
...
for ISL in "${ISLS[@]}"; do
    for CONC in "${CONCS[@]}"; do
        RESULT_FILENAME="pd-atom-kimi-k25-1p1d-tp4-${ISL}-${OSL}-${CONC}-${RANDOM_RANGE_RATIO}"
        echo ""
        echo "========================================="
...
json_files = sorted(result_dir.glob('pd-atom-kimi-k25-1p1d-tp4-*.json'))
if not json_files:
